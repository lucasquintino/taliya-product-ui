...
| Internal vendor package sync | Blocked by release drift | The current library artifacts are `0.1.1`, while `taliya-internal` still has the previously installed `0.1.0` vendor artifacts; `consumer-package-sync:audit` and `consumer-vendor-versioning:audit` fail until the consumer is refreshed. |
| Internal vendor package versioning | Blocked by release drift | The current consumer vendor artifacts are tracked, but they do not match the current `0.1.1` `dist-packages`; refresh after npm publication or an explicit local package refresh. |
...
| Aggregate readiness gate | Blocked by release/evidence gates | `corepack pnpm readiness:audit` is currently red because registry adoption is `0/3`, the current Internal vendor artifacts are `0.1.0`, and the synthetic fixture needs a Node-compatible typecheck run. |
...
| Future CRM capability handoff | Proven for current scope | `specs/001-product-ui-foundation/future-crm-adoption-handoff.md` defines candidate discovery criteria, bootstrap sequence, labeled readiness evidence, and the conditional adoption rule; `corepack pnpm future-crm-adoption-handoff:audit` checks the handoff and links from consumer docs. |
...
| Component/story architecture | Proven for current scope | `corepack pnpm components:audit` passes; 344 component stories have valid namespaces, and CRM primitive reuse reports 32 justified compound roots, 0 refactor rows, and 0 missing-primitive rows. |
...
| Source-image 1:1 certification | Incomplete globally | `corepack pnpm visual-certification-backlog:audit` reports 100 approved component/image rows, 63 semi-approved image rows, 1 image rows in visual review, 1 ignored image row, and 64 incomplete component/image certification rows. All 84/84 active image targets have explicit ledger rows; pending rows remain visible instead of silently disappearing from the backlog. |
...
- `visual-certification-backlog-audit.md` currently records 64 component/image certification rows that still block global 1:1 completion; each requires explicit source-backed capture evidence and a ledger verdict.
