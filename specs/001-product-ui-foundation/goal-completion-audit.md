...
| Taliya Internal owns the same consumer readiness config model future CRM will use | `C:/Users/lucas/taliya-internal/taliya-readiness.config.json`, `library-readiness-gate.json readinessConfig`, `consumer-readiness-config.example.json` | failed | Blocks readiness |
...
| Official library readiness handoff proves the packages can be consumed as the reusable UI source | `official-library-readiness:audit`, `official-library-readiness-audit.md`, `status=fail`, `officialConsumerReady=false`, `currentInternalReady=true`, `crmRealCanStart=false`, `realFutureCrmAdoptionExecuted=false`, `releaseCandidateGates=103`, `registryManualItems=3` | proven-for-current-scope | Satisfied for current scope |
...
| Release channel handoff proves the local install channel is reliable for consumers | `release-channel:audit`, `release-channel-audit.md`, `status=fail`, `localTarballChannelReady=false`, `registryReady=true`, `releasePolicyStatus=pass-registry-policy`, `currentVersion=0.1.0`, `consumerPackageSyncStatus=pass`, `consumerVendorVersioningStatus=pass`, `consumerConfigVersioningStatus=pass`, `registryBlockers=0` | proven-for-current-scope | Satisfied for current scope |
...
- Release channel readiness is `fail`, with local tarball channel ready: `false`, registry ready: `true`, and registry blockers: `0`.
