...
Official CRM page tables such as `TaskTable`, `ChecklistTable`, `ApprovalTable`, `StudentTable`, and `ReplacementTable` are data-presentational components. They accept prepared row objects, selected state, local display state (`source`, `loading`, `empty`, `blocked`), pagination labels and callbacks for row/page/filter actions. Domain row statuses remain explicit: `ChecklistTable` includes `pending`, `progress` and `overdue`; `ApprovalTable` includes `pending`, `expired`, `approved` and `rejected`. They may sort their provided rows locally for header affordance parity, but they must not fetch, route, persist, infer permissions, execute filters, open drawers by themselves, or own product workflow decisions.

`StudentTable` exposes the canonical student lifecycle statuses `active`, `paused`, `delinquent`, `risk`, `noClass`, and `inactive`, plus independent finance states `ok`, `pending`, and `overdue`. `StudentDrawer` mirrors the lifecycle through `active`, `paused`, `delinquent`, and `risk`, receives prepared `finance`, and emits `open-profile`, `message`, `create-task`, and `schedule` without coupling financial delinquency to attendance risk. `ClassDrawer` class-detail compositions use `open`, `conflict`, `open-class`, `view-demand`, and `edit-class` for the Turmas contract. `ReplacementTable` exposes `noVacancy`, `conflict`, `waiting`, `expired`, and `scheduled`; `ReplacementDrawer` owns the corresponding `no-vacancy`, `conflict`, `waiting`, `expired`, `scheduled`, and `consumed` display states and emits `find-fit`, `reserve-slot`, `send-invite`, and `consume-credit`. Consumers own state transitions and persistence.
...
`ChecklistDrawer` facts accept prepared display values plus optional `avatarSrc` for responsible/person facts. The drawer renders the avatar with the official `Avatar` primitive and keeps the image decorative inside the avatar wrapper; consumers provide any source image URL and own all person data. Its single action contract is `primaryActionLabel`/`onPrimaryAction`, `onAssign`, `onOpenTask`, `onComplete` and `onOpenOrigin`. Consumers select `Iniciar` or `Continuar` through the primary label without creating drawer variants. `state="completed"` disables execution mutations while leaving origin inspection available.

`TaskDrawer` owns the task lifecycle drawer anatomy. The `activityOrder="history-comments" | "comments-history"` prop controls the order of the history and comments sections while preserving the same drawer primitives; the default `history-comments` protects the activity/comment sequence. Drawer size and placement are not configurable by the domain component.
...
When a drawer is mounted, `CrmProductShell` always reserves the official drawer width and keeps the panel fixed to the right viewport edge from `top: 0` to `bottom: 0`. The dynamic viewport height token provides `100dvh`, the official medium drawer token provides width, and mobile uses full viewport width. Placement, side, inline, compact-width and wide-width variants are forbidden in public drawer APIs and consumer CSS.
