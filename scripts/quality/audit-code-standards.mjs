...
function add(ruleId, file, line, detail) {
  const fingerprint = crypto.createHash("sha256").update(`${ruleId}|${file}|${line}|${detail}`).digest("hex");
...
for (const sourceRoot of sourceRoots) {
  ...
  for (const relative of files) {
    ...
    lines.forEach((line, index) => {
      ...
      if (/\b(?:as\s+any|:\s*any\b|<any>)/.test(line)) add("STD-ANY-EXPLICIT", file, lineNumber, "explicit any");
      if (/@ts-(?:ignore|nocheck|expect-error)|eslint-disable/.test(line)) add("STD-SUPPRESSION", file, lineNumber, "suppression directive");
      if (/key\s*=\s*\{\s*(?:index|i)\s*\}/.test(line)) add("REACT-STABLE-KEY", file, lineNumber, "array index key");
      if (/\buseEffect\s*\(/.test(line)) add("REACT-EFFECT", file, lineNumber, "effect requires external synchronization justification");
      if (sourceRoot.includes("crm") && /<\s*(?:button|input|select|textarea)\b/.test(line)) add("REACT-RAW-CONTROL", file, lineNumber, "CRM source uses raw control instead of official primitive");
      if (/dangerouslySetInnerHTML|\beval\s*\(/.test(line)) add("SEC-UNSAFE-SINK", file, lineNumber, "unsafe HTML/evaluation sink");
    });
    ...
    if (!compatibilityRuntime && linesOfCode > 400) add("ARCH-MODULE-SIZE", file, 1, `logicalLines>${linesOfCode}`);
    if (!compatibilityRuntime && complexityProxy > 120) add("ARCH-COMPLEXITY", file, 1, `decisionProxy>${complexityProxy}`);
  }
}
