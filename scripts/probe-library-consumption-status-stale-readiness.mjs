...
const tempDir = mkdtempSync(join(tmpdir(), "taliya-library-consumption-stale-readiness-probe-"));
...
const evidence = {
  readiness: writeJson("library-readiness-gate.json", {
    status: "pass",
    rows: [{ id: "tokens", status: "pass" }]
  }),
...
if (result.status === 0) {
  console.error("Expected library consumption status to reject stale readiness evidence without required gates, but it passed.");
  process.exit(1);
}

const output = `${result.stdout ?? ""}\n${result.stderr ?? ""}`;
if (!output.includes("Failed library consumption rows: aggregate-readiness")) {
  console.error("Expected failure output to name aggregate-readiness.");
  console.error(output.trim());
  process.exit(1);
}

const reportPath = join(tempDir, "library-consumption-status-stale-readiness-probe.json");
if (!existsSync(reportPath)) {
  console.error("Expected stale readiness probe to write its labeled report in the temp directory.");
  process.exit(1);
}

const report = JSON.parse(readFileSync(reportPath, "utf8"));
if (report.status !== "fail" || report.aggregateReadinessPass !== false || report.missingReadinessGateIds.length === 0) {
  console.error("Expected stale readiness evidence to produce a failing status report with missing readiness gates.");
  console.error(JSON.stringify(report, null, 2));
  process.exit(1);
}

console.log("Library consumption status stale-readiness probe passed.");
