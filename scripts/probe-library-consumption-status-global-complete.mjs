...
const tempDir = mkdtempSync(join(tmpdir(), "taliya-library-consumption-complete-probe-"));
...
const evidence = {
  ...
  acceptance: writeJson("library-acceptance-audit.json", {
    ...
    globalGoalComplete: true
  }),
  goalCompletion: writeJson("goal-completion-audit.json", { verdict: "complete" }),
  ...
  visualBacklog: writeJson("visual-certification-backlog-audit.json", {
    summary: { globallyIncompleteCount: 0 }
  })
};
...
if (result.status !== 0) {
  console.error("Expected library consumption status to accept complete global evidence, but it failed.");
  console.error(`${result.stdout ?? ""}\n${result.stderr ?? ""}`.trim());
  process.exit(1);
}

const reportPath = join(tempDir, "library-consumption-status-complete-probe.json");
if (!existsSync(reportPath)) {
  console.error("Expected complete probe to write its labeled report in the temp directory.");
  process.exit(1);
}

const report = JSON.parse(readFileSync(reportPath, "utf8"));
if (
  report.status !== "pass-global-goal" ||
  report.futureCrmRealAdoptionExecuted !== true ||
  report.globalGoalComplete !== true ||
  report.counts.futureCrmCandidateCount !== 1 ||
  report.counts.adoptedFutureCrmCandidateCount !== 1
) {
  console.error("Expected complete evidence to produce pass-global-goal with future CRM adoption executed.");
  console.error(JSON.stringify(report, null, 2));
  process.exit(1);
}

console.log("Library consumption status global-complete probe passed.");
